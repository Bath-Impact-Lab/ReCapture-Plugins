...
from .processing import lowPassFilter
from .trc import trc_2_dict
...
class kinematics:
    ...
    def __init__(self, sessionDir, trialName, model_path, trcFilePath,
                 modelName=None,
        ...
        if model_path is not None:
            osim_dir = os.path.dirname(model_path)
            model_name = os.path.basename(model_path)
            modelName = os.path.splitext(model_name)[0]

        
        
        modelBasePath = osim_dir if osim_dir is not None else os.path.join(sessionDir, 'OpenSimData', 'Model')
        ...
        motionPath = os.path.join(sessionDir, '{}.mot'.format(trialName))
    ...
    def get_marker_dict(self, session_dir, trial_name, trcFilePath=None, 
                        lowpass_cutoff_frequency=-1):
        
        # Default path
        default_trc = os.path.join(
            session_dir,
            'MarkerData',
            f'{trial_name}.trc'
        )
        
        # Use provided path if valid, otherwise fallback
        if trcFilePath is not None and os.path.exists(trcFilePath):
            trcFilePath = trcFilePath
        elif os.path.exists(default_trc):
            trcFilePath = default_trc
        
    
        if trcFilePath is None:
            raise FileNotFoundError(f"No TRC file found for {trial_name}")
    ...
    def _normalize_side(self, side=None):
        if side in [None, 'default', 'primary']:
            if hasattr(self, 'primaryLeg'):
                return self.primaryLeg
            if hasattr(self, 'primarySide'):
                return self.primarySide
            if hasattr(self, 'defaultSide'):
                return self.defaultSide
            return 'r'
        return side.lower()

    def _get_first_available_marker(self, marker_names):
        for marker_name in marker_names:
            if marker_name in self.markerDict['markers']:
                return self.markerDict['markers'][marker_name]
        return None

    def _get_pelvis_reference(self):
    ...
    def _get_hip_center(self):
        pelvis_reference = self._get_pelvis_reference()
        right_hip = self._get_first_available_marker(['RHJC_study', 'RHJC', 'RHIP'])
        left_hip = self._get_first_available_marker(['LHJC_study', 'LHJC', 'LHIP'])

        hip_markers = [marker for marker in [right_hip, left_hip] if marker is not None]
        if not hip_markers:
            return pelvis_reference
        return np.mean(np.array(hip_markers), axis=0)

    def _get_angle_definitions(self):
        return {
            'ankle': {
                'coordinate': 'ankle_angle',
                'sided': True,
                'positive_label': 'dorsiflexion',
                'negative_label': 'plantar flexion'
            },
            'knee': {
                'coordinate': 'knee_angle',
                'sided': True,
                'positive_label': 'flexion',
                'negative_label': 'extension'
            },
            'hip': {
                'coordinate': 'hip_flexion',
                'sided': True,
                'positive_label': 'flexion',
                'negative_label': 'extension'
            },
            'pelvis_tilt': {
                ...
                'sided': False,
                'positive_label': 'anterior tilt',
                'negative_label': 'posterior tilt'
            },
            'pelvis_list': {
                'coordinate': 'pelvis_list',
                'sided': False,
                'positive_label': 'list',
                'negative_label': 'opposite list'
            },
            'pelvis_rotation': {
                'coordinate': 'pelvis_rotation',
                'sided': False,
                'positive_label': 'rotation',
                'negative_label': 'opposite rotation'
            },
            'trunk_flexion': {
            ...
            'trunk_bending': {
                ...
                'negative_label': 'opposite bending'
            },
            'trunk_rotation': {
            ...
            'shoulder_adduction': {
                'coordinate': 'arm_add',
                'sided': True,
                'positive_label': 'adduction',
                'negative_label': 'abduction'
            },
            'shoulder_rotation': {
                'coordinate': 'arm_rot',
                'sided': True,
    ...
    def _get_joint_coordinate_series(self, joint_config, side=None):
        coord_name = joint_config['coordinate']
        if joint_config['sided']:
            coord_name = f"{coord_name}_{self._normalize_side(side)}"
        return self.coordinate_values[coord_name] if hasattr(self, 'coordinate_values') and coord_name in self.coordinate_values.columns else self.coordinateValues[coord_name]
